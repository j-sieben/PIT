...
as
  ...
  */
  ...
  /* Method to set the default settings for languages
   * %param  p_pml_list   P_DELIMITER-separated list of languages in default order.
   *                      If P_PML_LIST is NULL, the language list resets to 
   *                      default language.
   * %param [p_delimiter] Optional delimiter to split the language list. Default: ':'
   * %usage  Method is used to adjust the order of languages. Default language cannot
   *         be changed (this is set during installation), but if you provide a list
   *         of languages, this will define the order in which PIT will try to
   *         instantiate the language for a message
   */
