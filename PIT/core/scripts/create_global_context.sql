...
declare
   ...
   C_CTX constant varchar2(128 byte) := 'VERSION_CTX';
begin
   select count(*)
   ...
   if l_context_exists = 0 then
      execute immediate 'create context ' || C_CTX || ' using &UTIL_USER..utl_context accessed globally';
...
declare
   l_context_exists number;
   C_CTX constant varchar2(128 byte) := 'KISMON_CTX';
begin
   select count(*)
     into l_context_exists
     from dual
