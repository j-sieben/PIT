  ...
  */
  ...
  /** 
    Procedure: handle_validation
      Exception handler that handles validation errors, warnings and hints
      
      Call this procedure in EXCEPTION handlers of your code.
      <handle_validation> not only reports the error but handles it as well.
      This means that if you pass any error that occurred to this
      procedure, the error gets logged and handled.
      
      <handle_validation> includes a call to <leave>, so there is no need to call <leave> explicitly.
      
      This method does not accept any parameters, it is desinged so that it works
      with the preiviously generated message such as with pit.assert... methods. 
      There is no way to define new messages solely for the validation process.
   */
  procedure handle_validation;
  ...
  /** 
    Procedure: handle_exception
      ...
      Call this procedure in EXCEPTION handlers of your code.
      <handle_exception> not only reports the error but handles it as well.
      This means that if you pass any error that occurred to this
      procedure, the error gets logged and handled.
  ...
  /** 
    Procedure: handle_panic
      Exception handler that handles unexpected and unrecoverable errors
      
      Call this procedure in EXCEPTION handlers of your code.
      <handle_panic> not only reports the error but handles it as well.
      This means that if you pass any error that occurred to this
      procedure, the error gets logged and handled.
      
      <handle_panic> includes a call to <leave>, so there is no need to call <leave> explicitly.
   */
  procedure handle_panic;
  ...
  /** 
    ...
    Parameters:
      ...
      p_message_name - Optional name of the message. References package <MSG>
      p_msg_args - Optional list of replacement information
      p_affected_id - Optional id of an item a message relates to
      p_affected_ids - Optional list of parameters a message relates to 
 ...
 procedure set_context_value(
  ...
  /**
    Function: assert_is_ok
      Method to check whether an exception has occured during collection mode.
      
    Parameters:
      p_error_code - Optional error code to group validations. If set, it is checked whether
                     and exception occured _whithin validations for the same error code_. Useful
                     if more than one validation is used for a given attribute.
                     If NULL and P_ERROR_CODE_LIST is also NULL, any error within the actual
                     collection run will trigger the result.
      p_error_code_list - Optional list of error codes. If not null, it is checked whether
                          an exception occured within one of the mentioned groups.
      p_threshold - Flag to set the level that triggers the evaluation. Defaults to
                    LEVEL_ERROR, meaning that only errors and fatal errors trigger
                    the result to be not ok. If set to LEVEL_FATAL, errors do not
                    trigger the outcome.
    
    Returns:
      Flag to indicate whether a message of leve P_THRESHOLD or more severe has
      occured during the actual collection run. Returns TRUE, if this has happend
      and FALSE otherwise.
   */
  function assert_is_ok(
    p_error_code in varchar2 default null,
  ...
  /**
    ...
    Returns:
      Flag to indicate whether a message of leve P_THRESHOLD or more severe has
      occured during the actual collection run. Returns FALSE, if this has happend
      and TRUE otherwise.
   */
  function assert_is_not_ok(
    p_error_code in varchar2 default null,
