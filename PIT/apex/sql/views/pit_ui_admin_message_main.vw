create or replace view pit_ui_admin_message_main as
  with message_raw as(
       select distinct pms.rowid row_id, pms_name, pms_pmg_name, pms_pml_name, pms_pse_id, pms_custom_error,
              dbms_lob.substr(pms_text, 4000, 1) pms_text, src.name pms_used_at,
              case when src.name like 'PIT%' or src.name in ('UTL_CONTEXT', 'PARAM', 'PARAM_ADMIN') then 'Y' else null end pms_is_system_message
         from dl_pit_message pms
         join all_source src
           on instr(upper(text), pms_name) > 0
        where owner in ('&INSTALL_USER.', '&APP_USER.')),
       messages as (
       select row_id, pms_name, pms_pmg_name, pms_pml_name, pms_pse_id, pms_custom_error, pms_text, 
         ...
         from message_raw
        group by row_id, pms_name, pms_pmg_name, pms_pml_name, pms_pse_id, pms_custom_error, pms_text)
select row_id, 
       pms_name,
       pms_pmg_name,
       pml_display_name,
  ...
  from messages pms
  join dl_pit_message_language pml
    on pms.pms_pml_name = pml.pml_name
  join dl_pit_message_severity pse
