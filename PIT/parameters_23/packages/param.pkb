...
as
  ...
  begin
    ...
    l_is_modifiable := l_row.par_id is not null and l_row.par_is_modifiable = true;
    ...
    if l_is_modifiable then
      ...
      l_row.par_boolean_value := p_par_boolean_value;
  ...
  begin
    l_row := get_parameter(p_par_id, p_par_pgr_id);
    return l_row.par_boolean_value = true;
  end get_boolean;
  ...
  as
    cursor local_param_cur is
      select pal_id, pal_pgr_id, pal_string_value, 
             ...
             case pal_boolean_value when true then 'true' when false then 'false' else 'null' end pal_boolean_value
