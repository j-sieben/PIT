...
as
  ...
  begin
    l_boolean := case when p_value then true
                      when not p_value then false
                      else null end;
    return l_boolean;
  end convert_boolean;
...
~';
  ...
  begin
    select replace(replace(replace(c_group_template,
                 ...
                 '#PGR_IS_MODIFIALBLE#', case
                                           when pgr_is_modifiable = true then 'true'
                                           when pgr_is_modifiable = false then 'false'
                                          end)
      into l_chunk
  ...
  as
    cursor realm_cur(p_pgr_id in parameter_group.pgr_id%type) is
      select pre_id, 
             pre_description, 
             case pre_is_active when true then 'true' else 'false' end pre_is_active
  ...
  as
    cursor param_cur(p_pgr_id in parameter_group.pgr_id%type) is
      select par_id, par_pgr_id, par_description,
             ...
             case 
               when par_boolean_value = true then 'true' 
               when par_boolean_value = false then 'false'
              end par_boolean_value,
             case
               when par_is_modifiable = true then 'true'
               when par_is_modifiable = false then 'false'
    ...
    cursor local_param_cur(p_pgr_id in varchar2) is
      select pal_id par_id,
             ...
             case
               ...
               when pal_boolean_value is not null then case pal_boolean_value when false then 'false' else 'true' end
               else 'null'
             end par_value
...
^';
  ...
  as
    ...
    l_pgr_is_modifiable parameter_group.pgr_is_modifiable%type := false;
  begin
    select -- If group cannot be modified, override individual setting, otherwise parameter setting
           par_id,
           par_pgr_id,
           case
             when pg.pgr_is_modifiable = true then
               coalesce(p.par_is_modifiable, true)
             else
               false
           end is_modifiable,
           par_validation_string,
  ...
  as
    ...
    l_valid pls_integer;
  begin
    select coalesce(max(pgr_is_modifiable), true)
      into l_pgr_is_modifiable
      from parameter_group
     where pgr_id = p_row.par_pgr_id;
     
    if l_pgr_is_modifiable = true or g_import_mode then
      -- Validiere Parameter
      if p_row.par_validation_string is not null then
        ...
        l_stmt := replace(l_stmt, '#BOOLEAN#', case when coalesce(p_row.par_boolean_value, true) = true then dbms_assert.enquote_literal(true) else dbms_assert.enquote_literal(false) end);
        $ELSE
  ...
  begin
    ...
    if l_parameter_rec.par_is_modifiable = true then
