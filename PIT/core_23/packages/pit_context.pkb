...
as
  ...
  begin
    l_module_meta := pit_module_meta(
                       p_module,
                       case when g_available_modules.exists(p_module) then true else false end,
                       case when g_active_modules.exists(p_module) then true else false end,
  ...
  as
    ...
    l_context pit_context_type;
  begin  
    if g_context.allow_toggle = true then
  ...
  begin
    read_active_context;
    return greatest(g_context.log_level, pit_internal.C_LEVEL_ERROR, pit_internal.C_LEVEL_SEVERE);
  end get_log_level;
  ...
  as
    ...
    l_module pit_util.ora_name_type;
    l_module_available boolean;
    l_module_active boolean;
