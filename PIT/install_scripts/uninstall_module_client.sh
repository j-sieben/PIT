...
MODULE="$(pit_require_value "${MODULE:-}" "${PIT_MODULE:-}" "Enter module name to revoke: ")"
LOG_FILE="$(pit_log_path "Uninstall_PIT_module_${MODULE}_client_${REMOTEOWNER}.log")"

pit_prepare_log "${LOG_FILE}"
pit_run_sqlplus "${OWNER}" "${OWNERPWD}" "${SERVICE}" "./install_scripts/revoke_module_client.sql" "${LOG_FILE}" "${OWNER}" "${REMOTEOWNER}" "${MODULE}"
pit_run_sqlplus "${REMOTEOWNER}" "${REMOTEPWD}" "${SERVICE}" "./install_scripts/unregister_module_client.sql" "${LOG_FILE}" "${OWNER}" "${REMOTEOWNER}" "${MODULE}"
