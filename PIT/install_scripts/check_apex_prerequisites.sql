...
begin
  dbms_output.put_line('Check whether PIT is accessible');
  select count(*)
    into l_pkg_text_exists
    from dual
   where exists(
         select null
           from all_objects
  ...
  if l_pkg_text_exists = 0 then
    dbms_output.put_line('&s1.PIT is not accessible. Please check whether you selected the correct PIT owner schema.');
