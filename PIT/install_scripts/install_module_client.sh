...
echo -n "Enter module name to grant [ENTER] "
read MODULE
echo ${MODULE}
...
@./install_scripts/grant_module_access.sql ${OWNER} ${REMOTEOWNER} ${MODULE}
EOF

sqlplus -L /nolog <<EOF
connect ${REMOTEOWNER}/"${REMOTEPWD}"@${SERVICE}
@./install_scripts/register_module_client.sql ${OWNER} ${REMOTEOWNER} ${MODULE}
EOF
