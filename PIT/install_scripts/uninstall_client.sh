...
LOG_FILE="$(pit_log_path "Uninstall_PIT_client_${REMOTEOWNER}.log")"

pit_prepare_log "${LOG_FILE}"
pit_run_sqlplus "${OWNER}" "${OWNERPWD}" "${SERVICE}" "./install_scripts/revoke_client.sql" "${LOG_FILE}" "${OWNER}" "${REMOTEOWNER}"
pit_run_sqlplus "${REMOTEOWNER}" "${REMOTEPWD}" "${SERVICE}" "./install_scripts/unregister_client.sql" "${LOG_FILE}" "${OWNER}" "${REMOTEOWNER}"
